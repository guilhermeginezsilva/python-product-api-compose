...
SQL_QUERY_SELECT_ALL_PRODUCTS = "SELECT ID, NAME, PRICE, URL FROM TB_PRODUCT"
SQL_QUERY_SELECT_PRODUCT = "SELECT ID, NAME, PRICE, URL FROM TB_PRODUCT WHERE ID = %s"
SQL_QUERY_INSERT_PRODUCT = "INSERT INTO TB_PRODUCT (ID, NAME, PRICE, URL) VALUES (%s, %s, %s, %s)"
SQL_QUERY_UPDATE_PRODUCT = "UPDATE TB_PRODUCT SET NAME=%s, PRICE=%s, URL=%s WHERE ID = %s"
SQL_QUERY_DELETE_PRODUCT = "DELETE FROM TB_PRODUCT WHERE ID = %s"
...
def find_all() -> []:
    ...
    try:
        ...
        with conn:
            cursor = conn.cursor()
            cursor.execute(SQL_QUERY_SELECT_ALL_PRODUCTS)
        ...
        logging.error("ERROR: Unexpected error: Could not connect to MySql instance: {}".format(error))
        if conn:
            conn.rollback()
        raise error

    except BaseException as error:
        logging.error("ERROR: Unexpected error: {}".format(error))
        if conn:
            conn.rollback()
        raise error
...
def find_one(product_id: str) -> Product:
    conn = None
    try:
        conn = pymysql.connect(**environment_config.get_db_config())
        ...
        with conn:
            cursor = conn.cursor()
            cursor.execute(SQL_QUERY_SELECT_PRODUCT, product_id)
...
def insert(product: Product):
    ...
    try:
        ...
        with conn:
            cursor = conn.cursor()
            cursor.execute(SQL_QUERY_INSERT_PRODUCT, (product.id, product.name, product.price, product.url))
...
def update(product: Product):
    ...
    try:
        ...
        with conn:
            cursor = conn.cursor()
            cursor.execute(SQL_QUERY_UPDATE_PRODUCT, (product.name, product.price, product.url, product.id))
...
def delete(product_id: str):
    ...
    try:
        ...
        with conn:
            ...
            cursor.execute(SQL_QUERY_DELETE_PRODUCT, product_id)
            conn.commit()
